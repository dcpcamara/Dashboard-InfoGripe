...
ui <- fluidPage(
  ...
  fluidRow(
    column(1, img(src = "logo.png", class = "logo-banner")),  # Logo in the first column
  ...
  tabsetPanel(
    ...
    tabPanel("Unidades Federativas",
             ...
             h3("Figura 1. Série temporal, estimativa de casos recentes de SRAG e tendências de curto (últimas 3 semanas) e longo prazo (últimas 6 semanas) por Unidade Federativa - geral e por faixas etárias de interesse.",
                class = "figure-title"),
             fluidRow(
               column(6, imageOutput("state_image1", width = "100%", height = "auto")),
               column(6, imageOutput("state_image2", width = "100%", height = "auto"))
             ),
    ...
    tabPanel("Capitais",
             ...
             h3("Série temporal, estimativa de casos recentes de SRAG e tendências de curto (últimas 3 semanas) e longo prazo (últimas 6 semanas) por Unidade Federativa - geral e por faixas etárias de interesse.", 
                class = "figure-title"),
             fluidRow(
               column(6, imageOutput("capital_image1", width = "100%", height = "auto")),
               column(6, imageOutput("capital_image2", width = "100%", height = "auto"))
             )
    )
  )
)
...
server <- function(input, output) {
  ...
  output$trends_figure1 <- renderImage({
    list(src = "www/Mapa_ufs_tendencia.png", contentType = "image/png", class = "responsive-img", alt = "Trends Figure 1")
  }, deleteFile = FALSE)
  
  output$trends_figure2 <- renderImage({
    list(src = "www/Mapa_capitais_tendencia.png", contentType = "image/png", class = "responsive-img", alt = "Trends Figure 2")
  }, deleteFile = FALSE)
  
  ### Tab 2: Brazil Static Figures ###
  output$brazil_figure1 <- renderImage({
    list(src = "www/fig_BR.png", contentType = "image/png", class = "responsive-img", alt = "Figure 1 for Brazil")
  }, deleteFile = FALSE)
  
  output$brazil_figure2 <- renderImage({
    list(src = "www/fig_BR_fx_etaria.png", contentType = "image/png", class = "responsive-img", alt = "Figure 2 for Brazil")
  }, deleteFile = FALSE)
  
  output$brazil_figure3 <- renderImage({
    list(src = "www/fig_BR_inc_mort.png", contentType = "image/png", class = "responsive-img", alt = "Figure 3 for Brazil")
  }, deleteFile = FALSE)
  
  output$brazil_figure4 <- renderImage({
    list(src = "www/fig_BR_virus_lab_inc.png", contentType = "image/png", class = "responsive-img", alt = "Figure 4 for Brazil")
  }, deleteFile = FALSE)
  
  output$brazil_figure5 <- renderImage({
    list(src = "www/fig_BR_virus_lab_mort.png", contentType = "image/png", class = "responsive-img", alt = "Figure 5 for Brazil")
  }, deleteFile = FALSE)
  
  output$brazil_figure6 <- renderImage({
    list(src = "www/fig_BR_virus_lab.png", contentType = "image/png", class = "responsive-img", alt = "Figure 6 for Brazil")
  }, deleteFile = FALSE)
  ...
  output$state_image1 <- renderImage({
    ...
    list(src = get_image_path(state, 1), contentType = "image/png", class = "responsive-img", alt = paste("State", state, "Image 1"))
  }, deleteFile = FALSE)
  
  output$state_image2 <- renderImage({
    state <- input$selected_state
    list(src = get_image_path(state, 2), contentType = "image/png", class = "responsive-img", alt = paste("State", state, "Image 2"))
  }, deleteFile = FALSE)
  
  output$state_image3 <- renderImage({
    state <- input$selected_state
    list(src = get_image_path(state, 3), contentType = "image/png", class = "responsive-img", alt = paste("State", state, "Image 3"))
  }, deleteFile = FALSE)
  ...
  output$capital_image1 <- renderImage({
    ...
    list(src = paste0(get_capital_image_path(capital), ".png"), contentType = "image/png", class = "responsive-img", alt = paste("Capital", capital, "Image 1"))
  }, deleteFile = FALSE)
  
  output$capital_image2 <- renderImage({
    capital <- input$selected_capital
    list(src = paste0(get_capital_image_path(capital), "_fx_etaria.png"), contentType = "image/png", class = "responsive-img", alt = paste("Capital", capital, "Image 2"))
  }, deleteFile = FALSE)
}
